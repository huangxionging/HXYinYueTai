...
// 获取悦单列表
#define GET_PLAYLIST (@"http://mapi.yinyuetai.com/playlist/show.json")

@interface HXMusicViewController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation HXMusicViewController
...
- (void) connectNetWork
{
    @synchronized(self)
    {
        [_manager GET: GET_PLAYLIST parameters: @{@"deviceinfo" : DEVICEINFO, @"id" : [NSString stringWithFormat: @"%@", _playListID]} success:^(AFHTTPRequestOperation *operation, id responseObject)
        {
